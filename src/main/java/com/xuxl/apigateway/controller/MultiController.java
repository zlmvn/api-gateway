...
import com.xuxl.apigateway.code.BizException;
import com.xuxl.apigateway.utils.JsonUtil;
...
public class MultiController {
	...
	public BaseResponse<Object> multi(HttpServletRequest request, @PathVariable String prefix,
		...
		if (apiInfo == null) {
			...
			throw new BizException(SystemReturnCode.UNKNOWN_METHOD_ERROR);
		}
		...
		if (!requestMethod.equalsIgnoreCase(type)) {
			...
			throw new BizException(SystemReturnCode.REQUEST_METHOD_ERROR);
		}
		Object proxy = apiInfo.getProxy();
		if (proxy != null) {
			logger.error(String.format("%s参数没有对应的处理器", mt));
			throw new BizException(SystemReturnCode.DUBBO_SERVICE_NOTFOUND_ERROR);
		}
	...
	private Object[] parseParamater(ApiParameterInfo[] parameters, HttpServletRequest request) throws BizException {
		if (parameters == null) {
			return null;
		} else {
			...
			for (int i = 0, size = parameters.length; i < size; i++) {
				...
				if (!isSimpleType(clazz)) {
					...
					if (Map.class.isAssignableFrom(clazz)) {
						...
						throw new BizException(SystemReturnCode.PARAMETER_ERROR);
					}
					// 不支持接口
					...
					try {
						if (StringUtils.isEmpty(value)) {
							...
							objects[i] = JsonUtil.deserialize(value.getBytes(), clazz);
						}
					} catch (Exception e) {
	...
	private Object fillValue(String value, Class<?> clazz, Class<?> genericParameterType) {
		if (clazz.isAssignableFrom(int.class) || clazz.isAssignableFrom(Integer.class)) {
			...
			if (BeanUtils.isSimpleProperty(genericParameterType)) {
				...
				return JsonUtil.deserialize(value.getBytes(),clazz);
			}
		} else if (Collection.class.isAssignableFrom(clazz)) {
			if (BeanUtils.isSimpleProperty(genericParameterType)) {
				return getTypeConverter(null).convertIfNecessary(value, clazz);
			} else {
				return JsonUtil.jsonToObjectHashMap(value, clazz, genericParameterType);
			}
		}
		return null;
